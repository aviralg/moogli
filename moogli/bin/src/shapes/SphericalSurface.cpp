...
EllipsoidalSurface::EllipsoidalSurface(const string & id) : Shape(id)
{
...
EllipsoidalSurface::EllipsoidalSurface( const string & id
...
EllipsoidalSurface::allocate()
{
    ...
    _geometry -> setNormalArray(new osg::Vec3Array(_vertices * _vertices), osg::Array::BIND_PER_VERTEX);
    _geometry -> setColorArray(new osg::Vec4Array(1), osg::Array::BIND_OVERALL);
    _geometry -> addPrimitiveSet(new osg::DrawElementsUShort(GL_TRIANGLES, 6 * (_vertices - 1) * (_vertices - 1)));
...
void
EllipsoidalSurface::set_color(const osg::Vec3f & color)
{
  _color = color;
